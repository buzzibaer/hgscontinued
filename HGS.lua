...
function HGS_MiniMapClick(self, button)
...
function GroupBox_SetName(self)
    if(self:GetText() == "") then
        HGSData[Instance][self:GetID()]["Name"] = "Group"..self:GetID();
    else
        HGSData[Instance][self:GetID()]["Name"] = self:GetText();
    end
    HGSGroup1Text:SetText(HGSData[Instance][1]["Name"]);
    HGSGroup2Text:SetText(HGSData[Instance][2]["Name"]);
    HGSGroup3Text:SetText(HGSData[Instance][3]["Name"]);
    ...
    HGSGroup7Text:SetText(HGSData[Instance][7]["Name"]);
    HGSGroup8Text:SetText(HGSData[Instance][8]["Name"]);
    HGSActiveText:SetText(HGSData[Instance][self:GetID()]["Name"]);
    HGSData[Instance]["Active"] = self:GetID();
end

function Healer_OnClick(self)
    local Healer = HGSData["Healers"][self:GetID()]["Name"];
    local Class = HGSData["Healers"][self:GetID()]["Class"];
    local HGroupID = HGSData[Instance]["Active"];

    if(self:GetID() <= HealerCount or self:GetID() >= 25) then
        if(HGSData[Instance][HGroupID]["Count"] < 12) then
            HGSData["Healers"][self:GetID()]["IsChosen"][Instance] = 1;
            HGS_AddHealer(Healer,Class,HGroupID); 
        end
    end
end

function Group_OnClick(self)
    local HSlot;
    local HGroup = self:GetName();
    local HGroupID = self:GetID();
...
function HGS_SetActiveGroup(self)
    HGSActiveText:SetText(self:GetText());
    HGSData[Instance]["Active"] = self:GetID();
end

function HGS_SetChannel(self)
    HGSData["Channel"] = self:GetText();
    --HGSCurrChan:SetText("sending to "..HGSData["Channel"]);
    HGSChanText:SetText("Channel: "..HGSData["Channel"]);
end
